...
Roughly speaking, the mass lost during hibernation ranges between 0.25 & 2

Roughly speaking, the number of arousals varies between 4 & 16

Slope is rise/run, which in this case is about 1.75 for rise and 12 for run, so we get 7/48 as our guesstimate of the slope.

Eyeballing the plot, we would also guess that the y-intercept is somewhere around 0.2
...
```{r}
model_bats <- lm(bats$mass_loss_g~bats$Arousals)
```
